...
dataName = 'CCV'; %%%flower17; caltech101_10base; flower102; CCV; caltech101_numofbasekernel_10; bbcsport2view
...
%         res_acc_10(ie,1:end-1) =  res(1,1:end-1);
%         res_nmi_10(ie,1:end-1) =  res(2,1:end-1);
%         res_pur_10(ie,1:end-1) =  res(3,1:end-1);
%         res_alg_10(ie,1:end-1) =  alignment(1:end-1);
        res_acc_10(ie,1:end-1) =  res(1,:);
        res_nmi_10(ie,1:end-1) =  res(2,:);
        res_pur_10(ie,1:end-1) =  res(3,:);
        res_alg_10(ie,1:end-1) =  alignment;
        load([path,'work2016/myAbsentMKCres1/',dataName,'_missingRatio_',num2str(epsionset(ie)),'_norm_',num2str(qnorm),...
            '_clustering_alg8_iter_',num2str(iter),'.mat'],'res','alignment');
        res_acc_10(ie,end) = res(1,end);
        res_nmi_10(ie,end) = res(2,end);
        res_pur_10(ie,end) = res(3,end);
        res_alg_10(ie,end) = alignment(end);
    end
